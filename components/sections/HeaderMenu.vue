...
<div class="flex fixed inset-0 z-20 font-tenor-sans " v-if="show">
    ...
    <transition name="slideleft">
        <div :class="$store.state.showHeaderMenu ? 'move-in' : 'move-out'" class="transition-all delay-500 max-w-full relative flex flex-col bg-gray-100 w-80">
                ...
                <ul v-for="(item, i) in menu_right.items" :key="i" class="flex flex-col text-black text-13.6px  bg-white">
                    ...
                    <div v-if="item._id == activeId">
                        <div v-for="(item,i) in item.childrens" :key="i" class=" font-poppins">
                            ...
                            <ul class=" px-2 p-1" v-if="item.childrens && item.childrens.length > 0">
                                <li v-for="(child,ii) in item.childrens" :key="ii">
                                    <nuxt-link  class="mx-5 p-1   m-1 rounded-md  hover:bg-gray-50 hover:text-green-700 flex" :to="child.url">
                                        {{ child.text }}
                                    </nuxt-link>
                                </li>
                            </ul>
            ...
            <div class="">
                <!-- <router-link v-if="$settings.sections.header.icons.account" to="/account/orders" title="Account" class="item p-2  rounded-md mx-1 ">
                ...
                <router-link v-if="$settings.sections.header.icons.wishlist" to="/wishlist" title="Wishlist" class="item p-2  rounded-md mx-1 elative">
            ...
            <div>
                <ul v-for="(item, i) in otherMenu" :key="i" class="flex flex-col text-black text-13.6px  bg-white">
                    ...
                    <transition name="slide">
                    <div v-if="item._id == activeId">
                        <div v-for="(item,i) in item.childrens" :key="i" class="text-black text-13.6px font-poppins">
                            <a class="p-2 m-1 rounded-md  hover:text-green-700 flex" :href="item.url">{{item.text}}</a>
                            <ul class="p-2" v-if="item.childrens && item.childrens.length > 0">
                                <li v-for="(child,ii) in item.childrens" :key="ii">
                                    <a class="p-2 m-1 rounded-md hover:bg-gray-50 hover:text-green-700 flex" :href="child.url">
                                        {{ child.text }}
                                    </a>
                                </li>
                            </ul>
...
export default {
    data() {
        return {
            show: false,
            activeId: null,
            menu: this.$settings.sections.header.left_menu,
            menu_right: this.$settings.sections.header.right_menu,
            otherMenu: [
                {
                    _id: "lang",
